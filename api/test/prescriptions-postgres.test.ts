...
describe.skipIf(!databaseUrl)('Phase 7.1B PostgreSQL prescription schema guards', () => {
  beforeAll(async () => {
    pool = new Pool({ connectionString: databaseUrl });
    const current = await pool.query<{ name: string }>('SELECT current_database() AS name');
    if (current.rows[0]?.name !== 'cliniq_phase7_prescription_verify') throw new Error('Refusing Phase 7.1B integration tests outside cliniq_phase7_prescription_verify.');
    const schema = await pool.query("SELECT to_regclass('prescriptions') AS prescriptions,to_regclass('prescription_items') AS prescription_items,to_regclass('appointments') AS appointments");
    if (!schema.rows[0]?.prescriptions || !schema.rows[0]?.prescription_items || !schema.rows[0]?.appointments) throw new Error('Phase 7.1B migration prerequisites are not applied.');
  });
  afterAll(async () => { await pool?.end(); });

  it('creates the tables and enforces appointment, participant, status, and idempotency guards', async () => {
    const constraints = await pool!.query<{ foreign_keys: string; checks: string }>("SELECT count(*) FILTER (WHERE contype='f')::text AS foreign_keys,count(*) FILTER (WHERE contype='c')::text AS checks FROM pg_constraint WHERE conrelid IN ('prescriptions'::regclass,'prescription_items'::regclass)");
    expect(constraints.rows[0]).toMatchObject({ foreign_keys: '6' });
    expect(Number(constraints.rows[0]?.checks ?? '0')).toBeGreaterThanOrEqual(12);
    const guard = await pool!.query<{ definition: string }>("SELECT pg_get_functiondef('prescription_mutation_guard'::regproc) AS definition");
    expect(guard.rows[0]?.definition).toContain("participant_type='DOCTOR'");
    const fixture = await seed(pool!); const prescription = draft(fixture);
    await pool!.query(insertPrescription(), values(prescription));
    await expect(pool!.query(insertPrescription(), values({ ...prescription, id: randomUUID(), createKey: randomUUID() }))).rejects.toMatchObject({ code: '23505' });
    await expect(pool!.query(insertPrescription(), values({ ...prescription, id: randomUUID(), appointmentId: randomUUID(), createKey: randomUUID() }))).rejects.toMatchObject({ code: 'P0001' });
    await expect(pool!.query(insertPrescription(), values({ ...prescription, id: randomUUID(), patientId: randomUUID(), createKey: randomUUID() }))).rejects.toMatchObject({ code: 'P0001' });
  ...
  it('enforces item constraints, issuance prerequisite, and issued immutability', async () => {
    const fixture = await seed(pool!); const prescription = draft(fixture);
    await pool!.query(insertPrescription(), values(prescription));
    await expect(pool!.query("UPDATE prescriptions SET status='ISSUED',issued_at=current_timestamp,issued_by_account_id=$2,issue_idempotency_key='issue',issue_request_fingerprint='issue',final_snapshot='{}'::jsonb,final_snapshot_hash=$3 WHERE id=$1", [prescription.id, fixture.doctorAccount, 'b'.repeat(64)])).rejects.toMatchObject({ code: 'P0001' });
    await expect(pool!.query(insertItem(), itemValues({ prescriptionId: prescription.id, position: 0 }))).rejects.toMatchObject({ code: '23514' });
    await pool!.query(insertItem(), itemValues({ prescriptionId: prescription.id, position: 1 }));
    await expect(pool!.query(insertItem(), itemValues({ prescriptionId: prescription.id, position: 1 }))).rejects.toMatchObject({ code: '23505' });
    await pool!.query("UPDATE prescriptions SET status='ISSUED',issued_at=current_timestamp,issued_by_account_id=$2,issue_idempotency_key='issue',issue_request_fingerprint='issue',final_snapshot='{}'::jsonb,final_snapshot_hash=$3 WHERE id=$1", [prescription.id, fixture.doctorAccount, 'c'.repeat(64)]);
    await expect(pool!.query("UPDATE prescriptions SET final_snapshot='{" + '"changed":true' + "}'::jsonb WHERE id=$1", [prescription.id])).rejects.toMatchObject({ code: 'P0001' });
    await expect(pool!.query('DELETE FROM prescriptions WHERE id=$1', [prescription.id])).rejects.toMatchObject({ code: 'P0001' });
    await expect(pool!.query("UPDATE prescription_items SET medicine_name='Changed' WHERE prescription_id=$1", [prescription.id])).rejects.toMatchObject({ code: 'P0001' });
    await expect(pool!.query('DELETE FROM prescription_items WHERE prescription_id=$1', [prescription.id])).rejects.toMatchObject({ code: 'P0001' });
  });
});

type Fixture = { appointmentId: string; patientId: string; doctorId: string; doctorAccount: string };
type Prescription = Fixture & { id: string; createKey: string; status?: string; issuedAt?: Date; issuedBy?: string; issueKey?: string; issueFingerprint?: string; snapshot?: object; hash?: string };

function draft(fixture: Fixture): Prescription { return { ...fixture, id: randomUUID(), createKey: randomUUID() }; }
function insertPrescription() { return `INSERT INTO prescriptions (id,appointment_id,patient_profile_id,doctor_profile_id,status,created_by_account_id,create_idempotency_key,create_request_fingerprint,issued_at,issued_by_account_id,issue_idempotency_key,issue_request_fingerprint,final_snapshot,final_snapshot_hash) VALUES ($1,$2,$3,$4,$5,$6,$7,'fixture',$8,$9,$10,$11,$12::jsonb,$13)`; }
function values(value: Prescription) { return [value.id, value.appointmentId, value.patientId, value.doctorId, value.status ?? 'DRAFT', value.doctorAccount, value.createKey, value.issuedAt ?? null, value.issuedBy ?? null, value.issueKey ?? null, value.issueFingerprint ?? null, value.snapshot ? JSON.stringify(value.snapshot) : null, value.hash ?? null]; }
...
async function seed(pool: Pool): Promise<Fixture> {
  ...
  try {
    ...
    await client.query('INSERT INTO appointment_financial_handoffs (id,appointment_intent_id,slot_reservation_id,financial_allocation_snapshot_id,payment_intent_id,booking_actor_account_id,idempotency_key,request_fingerprint) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)', [ids.handoff, ids.intent, ids.reservation, ids.allocation, ids.payment, ids.patient, `handoff-${ids.intent}`, 'prescription-fixture']);
    ...
    await client.query('INSERT INTO appointment_participants (id,appointment_id,participant_type,patient_profile_id) VALUES ($1,$2,$3,$4)', [randomUUID(), ids.appointment, 'PATIENT', ids.patient]);
    await client.query('INSERT INTO appointment_participants (id,appointment_id,participant_type,doctor_profile_id) VALUES ($1,$2,$3,$4)', [randomUUID(), ids.appointment, 'DOCTOR', ids.doctor]);
    ...
    await client.query("UPDATE appointments SET status='IN_PROGRESS' WHERE id=$1", [ids.appointment]);
    await client.query("INSERT INTO audit_events (id,category,event_type,target_type,target_id,outcome,metadata) VALUES ($1,'BUSINESS','FIXTURE_APPOINTMENT_TRANSITION','APPOINTMENT',$2,'SUCCESS','{}'::jsonb)", [ids.startAudit, ids.appointment]);
    await client.query("INSERT INTO appointment_events (id,appointment_id,event_type,previous_status,resulting_status,context,audit_event_id) VALUES ($1,$2,'STARTED','CONFIRMED','IN_PROGRESS','{}'::jsonb,$3)", [ids.startEvent, ids.appointment, ids.startAudit]);
    await client.query('COMMIT'); return { appointmentId: ids.appointment, patientId: ids.patient, doctorId: ids.doctor, doctorAccount: ids.doctorAccount };
