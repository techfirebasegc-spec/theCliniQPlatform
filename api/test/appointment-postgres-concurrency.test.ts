...
describe.skipIf(!enabled)('Phase 5 Step 3.2 real PostgreSQL reservation concurrency', () => {
  ...
  it('serializes two genuine concurrent capacity=1 reservations and releases capacity safely', async () => {
    const now = new Date(); const requestedLocalAt = nextMondayUtc(now);
...
function nextMondayUtc(now: Date) { const value = new Date(now); const days = (8 - value.getUTCDay()) % 7 || 7; value.setUTCDate(value.getUTCDate() + days); value.setUTCHours(10, 0, 0, 0); return value.toISOString().slice(0, 19); }
async function seed(target: Pool) {
  ...
  await target.query('INSERT INTO availability_configurations (id,service_offering_version_id,provider_timezone,slot_duration_seconds,buffer_before_seconds,buffer_after_seconds,capacity,booking_lead_time_seconds,booking_horizon_seconds,status,created_by_account_id) VALUES ($1,$2,\'UTC\',1800,0,0,1,0,1209600,\'ACTIVE\',$3)', [configuration, version, doctorAccount]);
  ...
  await target.query('INSERT INTO availability_windows (id,availability_rule_id,kind,weekday,start_seconds,end_seconds,created_by_account_id) VALUES ($1,$2,\'WORKING\',1,0,86400,$3)', [window, rule, doctorAccount]);
}
async function cleanup(target: Pool) {
  ...
  await target.query('DELETE FROM service_offering_version_reservation_policies WHERE id=$1', [policy]); await target.query('DELETE FROM service_offering_prices WHERE id=$1', [price]); await target.query('DELETE FROM service_offering_versions WHERE id=$1', [version]); await target.query('DELETE FROM service_exposures WHERE id=$1', [exposure]); await target.query('DELETE FROM service_offerings WHERE id=$1', [offering]);
  await target.query('DELETE FROM doctor_profiles WHERE id=$1', [doctorProfile]); await target.query('DELETE FROM patient_profiles WHERE account_id = ANY($1::uuid[])', [[patientA, patientB]]); await target.query('DELETE FROM accounts WHERE id = ANY($1::uuid[])', [[patientA, patientB, doctorAccount]]);
}
