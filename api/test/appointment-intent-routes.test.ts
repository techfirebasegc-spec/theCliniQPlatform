...
describe('Phase 5 Step 3.2 appointment intent routes', () => {
  ...
  it('fails closed when missed-webhook recovery is requested without Razorpay configuration', async () => { const { app } = setup(); const response = await app.inject({ method: 'POST', url: '/v1/appointment-intents/intent-a/payment-recovery', headers: { cookie: 'cliniq_session=session-a.secret' } }); expect(response.statusCode).toBe(503); expect(response.json()).toMatchObject({ error: { code: 'PROVIDER_NOT_CONFIGURED' } }); await app.close(); });
});
